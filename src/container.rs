    ...
    match command {
        ...
        ContainerCommand::Start {
         ...
         => {
            ...
            start(container.to_string()).await;
        }
...
pub async fn ps(all: bool) {
    ...
    pub async fn start(id: String) {
        ...
        match container.start().await {
            Ok(()) => println!("successful started container"),
            Err(e) => eprintln!("Error: {}", e),
            // _ => println!("unknown"),
        }
    }

    pub async fn stop(id: String, time: Option < u64 > ) {
        let docker = Docker::new();
        let mut container = docker
            .containers()
            .get( & id);
        // match container.stop().await {
        // Ok(()) => println!("starting  Container"),
        // Err(e) => eprintln!("Error: {}", e),
        // }
        if let Some(t) = time {
            let duration_time = Duration::new(t, 0);
            let result = container.stop(Some(duration_time)).await;
            println!("successful stopped {:?}", result);
        }
        else {
            let result = container.stop(None).await;
